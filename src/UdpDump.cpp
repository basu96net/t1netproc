...
void packet_handler_udpdump(u_char *param, const struct pcap_pkthdr *header, const u_char *pkt_data);


int mainUdpDump(int argc, char* argv[])
{
	pcap_if_t *alldevs;
	pcap_if_t *dev;
	int inum;
	...
	/* start the capture */
	pcap_loop(adhandle, 0, packet_handler_udpdump, NULL);
