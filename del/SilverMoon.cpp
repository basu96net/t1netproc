...
int mainSimon(int argc, char * argv[])
{
    u_int i, res , inum ;
    ...
    const u_char  ** tmp = &pkt_data;
    while((res = pcap_next_ex( fp, &header, tmp)) >= 0)
    {
        if(res == 0)
        {
            // Timeout elapsed
...
void PrintTcpPacket(const u_char* Buffer, int Size)
{
    ...
    data = const_cast<unsigned char *> ( Buffer + sizeof(ETHER_HDR) + iphdrlen + tcphdrlen );
...
void print_udp_packet(const u_char *Buffer,int Size)
{
    ...
    data = const_cast<u_char*>( Buffer + sizeof(ETHER_HDR) + iphdrlen + sizeof(UDP_HDR) );
...
void PrintIcmpPacket(const u_char* Buffer , int Size)
{
    ...
    data = const_cast<unsigned char *> ( Buffer + sizeof(ETHER_HDR) + iphdrlen + sizeof(ICMP_HDR) );
